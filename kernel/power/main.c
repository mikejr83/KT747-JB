					...
					const char *buf, size_t n)
{
	...
	if (freq_min_limit == -1) {
		cpufreq_set_limit(
			APPS_MIN_STOP,
			GLOBALKT_MIN_FREQ_LIMIT);
		freq_min_apps = GLOBALKT_MIN_FREQ_LIMIT;
		...
		if (table != NULL) {
			for (i = 0;
				...
				if (table[i].frequency == freq_min_limit) {
					if (freq_min_limit <= GLOBALKT_MIN_FREQ_LIMIT) {
						cpufreq_set_limit(
							APPS_MIN_STOP,
							GLOBALKT_MIN_FREQ_LIMIT);
						freq_min_apps = GLOBALKT_MIN_FREQ_LIMIT;
						...
						cpufreq_set_limit(
							APPS_MIN_START,
							GLOBALKT_MAX_FREQ_LIMIT);
						freq_min_apps = GLOBALKT_MAX_FREQ_LIMIT;
						freq_min_apps_lock = 1;
					} else {
						cpufreq_set_limit(
							APPS_MIN_START,
	...
	if (freq_min_limit >= GLOBALKT_MAX_FREQ_LIMIT)
		dual_boost(1);
	else
		dual_boost(0);
#endif
					...
					const char *buf, size_t n)
{
	...
	if (freq_max_limit == -1) {
		cpufreq_set_limit(
			APPS_MAX_STOP,
			GLOBALKT_MAX_FREQ_LIMIT);
		freq_max_apps = GLOBALKT_MAX_FREQ_LIMIT;
		...
		if (table != NULL) {
			for (i = 0;
				...
				if (table[i].frequency == freq_max_limit) {
					if (freq_max_limit <= GLOBALKT_MIN_FREQ_LIMIT) {
						cpufreq_set_limit(
							APPS_MAX_START,
							GLOBALKT_MIN_FREQ_LIMIT);
						freq_max_apps = GLOBALKT_MIN_FREQ_LIMIT;
